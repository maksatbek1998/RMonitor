...
namespace Monitor.ViewModel
{
    class MainViewModel : BaseViews.BaseViewModel
    {
        ...
        public static int elementDuration;
        ...
        public void timeDelation(ref MediaElement obj)
        {
            ...
            dataBase.del += bd =>
            {
                if (bd.Rows.Count > 0)
                {
                    for (int i = 0; i < bd.Rows.Count; i++)
                    {
                        ...
                        {
                            element.Source = new Uri(bd.Rows[i][1].ToString());
                            element.Play();
            ...
            dataBase.SoursData("SELECT a.start_time,a.end_time,d.video_uri FROM ads_schedules AS a INNER JOIN ads AS d ON a.ads_id = d.id");
        }
